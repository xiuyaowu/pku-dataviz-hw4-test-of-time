...
Purpose: give D / Topic Evolution a report-ready answer to Issue #60: what changes across decades in the current Test-of-Time sample, which observations are safe, and which representative papers can be used as cases.

Data used: `data/papers_enriched.csv`, `data/topic_year_stats.csv`, and `data/topic_stats.csv` from the current repository. The companion machine-readable case table is `docs/topic_shift_narrative_cases.csv`.
...
The full case table is `docs/topic_shift_narrative_cases.csv`.
...
## D-owner remaining checks

- [ ] Choose 2-3 cases from `docs/topic_shift_narrative_cases.csv` for the final report.
