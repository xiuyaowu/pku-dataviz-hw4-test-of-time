...
更详细的数据来源和指标审计见：`docs/data_provenance_audit.md`。最终报告、展示稿和网页文案应统一采用其中的口径：公开元数据用于观察结构性模式，人工 annotation 用于解释单篇论文贡献；citation、breadth、institution/country counts 都不应被写成官方评奖原因或完整因果证据。
...
| Manual annotation | 队友人工阅读论文、award citation、项目主页和可信介绍后补充 contribution / evidence URL。 | 只有经过证据链接核查的内容才适合写成单篇论文贡献或工业影响判断。 |
