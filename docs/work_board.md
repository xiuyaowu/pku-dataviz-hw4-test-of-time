...
| B | [#3](https://github.com/leejamesss/pku-dataviz-hw4-test-of-time/issues/3) | Time：发表年、获奖年、recognition lag | `data/award_timeline.csv`, `data/recognition_lag_distribution.csv`, `src/app.js`, `docs/time_recognition_lag_handoff.md` | lag 分布解释、最长/典型 lag 代表论文、2-3 条时间发现；当前 handoff 已给出统一定义、代表论文候选、年代差异和可复制报告段落 | Time 两个图正常显示；结论能解释“时间检验”的时间尺度；最终报告前人工核查 3 篇代表论文的 award citation / 贡献表述 |
| C | [#4](https://github.com/leejamesss/pku-dataviz-hw4-test-of-time/issues/4) | Venue and Field：会议和领域分布 | `data/venue_stats.csv`, `data/venue_area_stats.csv`, `docs/venue_field_handoff.md`, Venue section | Top venues、Top fields、集中度分析、field × decade 读法、2-3 条会议/领域发现；当前 handoff 已给出核心统计、代表论文候选和可复制报告段落 | 不把数量榜误写成官方排名；说明设奖历史、奖项覆盖和数据源完整性限制；最终报告前人工核查 2-3 篇代表论文链接 |
...
| E | [#6](https://github.com/leejamesss/pku-dataviz-hw4-test-of-time/issues/6) | Citation and Impact：引用轨迹、深度和广度 | `data/citation_trajectories.csv`, `data/citing_breadth_metrics.csv`, `docs/citation_impact_handoff.md`, `docs/evidence_cards_top12.md`, `docs/impact_breadth_evidence_audit.md`, Citation section | citation vs lag、trajectory、depth × breadth、四象限代表论文、2-3 条影响力发现；当前 handoff 已给出核心统计、报告段落、展示话术和 proxy 边界，#62 audit 已把 Top 12 breadth 案例分成 safe/proxy/human-check 三类 | 明确区分 citation depth 和 breadth；说明 OpenAlex 近似指标限制；代表案例只使用已有 evidence URL 支撑的贡献表述 |
| F | [#7](https://github.com/leejamesss/pku-dataviz-hw4-test-of-time/issues/7) | Network / Visual / Presentation：机构国家、视觉统一和展示 | `data/institution_stats.csv`, `data/country_stats.csv`, Network section, CSS/PPT, `docs/live_demo_fallback_script.md` | institution/country 发现、可选网络图升级、视觉统一检查、PPT 大纲、live demo 失败时的截图 fallback | Network baseline 正常；全站风格不被破坏；展示结构和备用截图路线可直接用 |
