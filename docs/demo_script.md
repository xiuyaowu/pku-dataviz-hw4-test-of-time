...
展示 recognition lag histogram 和 publication → award timeline。

讲法：

> recognition lag 是从发表年到获奖年的间隔，它直接对应“时间检验”的时间尺度。我们可以看到，许多论文会经过较长时间后才被重新评价。极长 lag 的案例说明，有些基础研究需要等后续技术生态成熟后，其价值才会变得更明显。
...
展示 topic distribution、topic evolution、representative paper detail card。

讲法：

> 主题模块把论文从 venue 维度推进到内容维度。长期影响论文并不固定在单一主题，而是随计算机领域的技术重心变化。代表论文卡片用于把统计模式落回具体贡献，避免只展示抽象数量。
...
> 选中任意论文后，Benchmark Lab 会把它放回全数据集比较，显示 citation depth、impact breadth、recognition lag、国家和机构跨度分别处在什么 percentile。右侧 interpretation lens 会把它自动归类为 deep + broad influence、slow-burn classic、wide diffusion case 等类型。这样我们不只说“这篇论文重要”，而是能说明它相对数据集强在哪里。

## 8. Network：影响如何在研究生态中分布（约 1 分钟）

展示 institution 和 country/region ranking。

讲法：

> 最后我们把长期影响从单篇论文扩展到研究生态。机构和国家分布显示了长期影响的集中与扩散，但机构元数据并不完整，因此这里更适合作为结构性观察，不能替代完整合作网络统计。

## 9. 结尾：回答核心问题（约 40 秒）

讲法：

> 总结来看，经得起时间检验的研究通常需要多维观察。它可能经历较长 recognition lag，出现在特定学术社区和主题演化节点上，在引用轨迹中持续累积影响，并通过机构和地区网络扩散。我们的可视化把“长期影响”拆成时间、领域、主题、引用、证据检索、单篇基准比较和网络七个可观察维度，同时保留具体论文案例供进一步解释。
