...
This audit is a topic-label reference, not a substitute for reading the final DOI/ACM/IEEE/PDF evidence before submission.
...
Value: this turns Topic Evolution from a raw metadata chart into a defensible annotation workflow. The team can explicitly say that automatic topic labels were checked for representative papers, and that misleading labels were corrected before being used as evidence.
...
- Use `docs/analysis/manual_topic_audit_top12.csv` when selecting Topic Evolution examples for report/PPT.
...
See `docs/analysis/manual_topic_audit_top12.csv` for row-level fields:
