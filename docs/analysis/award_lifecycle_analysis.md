...
Purpose: provide a small, report-ready view of citation windows around Test-of-Time recognition. The analysis aligns papers by award announcement year (`t = 0`) and compares available yearly citation metadata before and after public recognition.
...
Careful wording boundary: these are OpenAlex-derived citation trajectory windows around announcement year. They can describe whether attention was rising, sustained, or cooling in the available metadata, but they do **not** prove that the award caused a citation change.

## Generated files

| File | Rows | Use |
|---|---:|---|
| `docs/analysis/award_lifecycle_prepost_metrics.csv` | 180 | Paper-level before / award-year / after citation-window metrics. |
| `docs/analysis/award_lifecycle_relative_year_summary.csv` | 15 | Relative-year aggregate table for an aligned mini-line chart or report table. |
