...
- 维护 clear 仓库结构，保证 README、docs、data、manual annotations、src 等目录对成员清晰可用。
...
- 编写小组工作分工板，把六人工作拆成 Issues、文件、交付物和 PR 合并标准。
- 编写模块任务卡，为 B-F 提供每个模块的已有 baseline、主要数据、最低交付、增强方向和报告句式。
- 编写 `docs/report/report_skeleton.md`，提供最终报告结构和各模块可直接填充的段落框架。
- 编写 6-8 分钟展示讲稿和备选 Q&A。
- 建立“继续提 feature、开 Issue、PR 完成、回填文档”的迭代循环，方便后续持续加功能。

### 5. GitHub 协作与 PR 流程

- 建立小组协作仓库，并清理为 clear 结构。
- 创建并细化 Issues #2–#7，对应 A-F 六个成员的工作流。
- 为每个 Issue 写清研究问题、已有 baseline、相关文件、继续优化任务、报告小节和验收标准。
- 建立 PR 模板，要求每个 PR 说明改动、数据、发现和验证方式。
- 制定 PR review 合并标准，让后续 review 有可追溯标准。
...
- 后续负责 review 成员 PR，检查是否链接对应 Issue、是否写清数据和发现。
...
- 最终整合 dry run：检查 clean main、本地服务器、presentation mode、DOM section checks 和核心数据行数。
- 最终提交前做全站 QA 检查。
...
A 负责项目整体架构、全站 baseline 实现与最终整合。前期完成 GitHub 协作仓库、Issue/PR 工作流、数据接入、D3 页面结构、五个研究模块的可运行图表、tooltip/详情卡/reading notes 和统一视觉系统；同时补齐 README demo 截图、成员任务卡、报告骨架和展示脚本，降低其他成员从零搭建的成本。中后期负责 review 各模块 PR、检查页面运行、数据路径、图表解释、术语统一和最终 QA，保证项目最终形成一条完整的数据故事，避免图表拼盘感。
