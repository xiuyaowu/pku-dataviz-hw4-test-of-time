...
Purpose: give the B / Time module a small, report-ready set of cases for explaining why `recognition_lag` is analytically useful. The cases below are computed from `data/papers_enriched.csv` and exported as a machine-readable companion table: `docs/recognition_lag_outlier_cases.csv`.
...
| Case type | Paper | Venue | Year → Award | Lag | Why it is useful | Safe wording boundary |
|---|---|---|---:|---:|---|---|
...
| Typical lag | `XORs in the air: practical wireless network coding` | SIGCOMM | 2006 → 2018 | 12 | Stable median-lag example already used in the Time handoff. | Final wording should verify the paper's contribution from award citation or paper abstract. |
...
- [ ] If using SIFT as a bridge into Citation / Benchmark, reuse the safe wording from evidence-card docs instead of inventing a new contribution claim.
